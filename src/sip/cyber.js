...
function isUnifiedPlanDefault() {
    ...
    if (!('currentDirection' in window.RTCRtpTransceiver.prototype))
        return false;
...
CyberMegaPhone.prototype.connect = function () {
    ...
    let socket = new JsSIP.WebSocketInterface('wss://' + this.host + ':10089/ws');
    ...
    this._ua.set("display_name", this.name);
    ...
    this._ua.on('newRTCSession', function (data) {
        ...
        rtc.interop = new sdp.Interop();

        console.log('new session - ' + rtc.direction + ' - ' + rtc);

        rtc.on("confirmed", function () {
            // ACK was received
            let streams = rtc.connection.getSenders().map(sender => sender.track);
            console.log("Streams: ", streams);
            // let streams = rtc.connection.getReceivers();
...
CyberMegaPhone.prototype.answer = function (rtc) {
    if (!this._ua) {
        return;
    }
...
export function mute(stream, options) {
...
export function unmute(stream, options) {
